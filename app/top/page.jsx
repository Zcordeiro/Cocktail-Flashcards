...
const topPage = () => {
    ...
    const fetchCocktails = async () => {
        const res = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${topCocktails[Math.floor(Math.random() * topCocktails.length)]}`);
    ...
    return (
        <div className="bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-gray-900 to-gray-400 w-full px-7 py-4 md:px-24 md:py-10 text-neutral-50 min-h-screen">
            ...
            {cocktails && cocktails.map((cocktail) => (
                <div key={cocktail} className="flex flex-col items-center justify-center" >
                    <Flashcard  cocktail={cocktail} />
                </div>
            ))}

            <button onClick={handleNext} className="fixed top-80 right-32 text-neutral-950 m-4 p-4 border-2 border-black rounded-lg bg-neutral-100 hover:bg-neutral-200 hover:font-bold">Next</button>
